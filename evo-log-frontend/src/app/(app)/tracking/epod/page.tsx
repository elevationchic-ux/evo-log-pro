    ...
    try {
      ...
      if (resEpod.ok) {
        ...
        if (data.facture_auto) {
          ...
          setInvoiceResult(null);
          toast.success('e-POD validé ; la facturation reste en attente du service finance.');
        }
      } else {
        setInvoiceResult(null);
        toast.error('L’e-POD n’a pas été validé par le service transport.');
      }
    } catch (err) {
      setInvoiceResult(null);
      toast.error('Le service e-POD est indisponible ; aucune facture n’a été créée.');
    } finally {
      setLoading(false);
    }
  };
