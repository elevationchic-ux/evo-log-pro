      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
        ...
        <div className="flex justify-between items-end mb-6">
          ...
          <div className="flex gap-2">
            <Link href="/magasin/declarations" className="px-4 py-2 rounded-xl text-sm font-semibold bg-slate-900 hover:bg-slate-700">
              Retour à la liste
            </Link>
          </div>
        </div>
        ...
        {declaration.statut && (
          <div className="mb-6 inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-blue-500/15 text-blue-300">
            {declaration.statut}
          </div>
        )}
        ...
        <div className="mb-8">
          ...
          <div className="bg-slate-900 rounded-2xl shadow-sm border border-slate-700 p-6">
            <div className="grid grid-cols-2 gap-6">
              <div>
                <h3 className="text-lg font-bold text-slate-200 mb-3">Identification</h3>
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-200 mb-3">Parties</h3>
            ...
            <div className="mt-6 p-4 bg-blue-500/10 rounded-xl border border-blue-500/40">
              <h3 className="text-lg font-bold text-slate-200 mb-3">Navire & Ports</h3>
        ...
        <div className="mb-8">
          ...
          <div className="bg-slate-900 rounded-2xl shadow-sm border border-slate-700 p-6">
            <div className="mb-4">
              <p className="text-sm font-medium text-slate-300"><strong>Code Article:</strong> <span className="font-mono text-blue-600">{declaration.code_article}</span></p>
              {declaration.lignes && declaration.lignes[0] && (
                <>
                  <p className="text-sm font-medium text-slate-300 mb-1"><strong>Désignation:</strong> {declaration.lignes[0].article?.nom}</p>
                  <p className="text-sm font-medium text-slate-300"><strong>Quantité Déclarée:</strong> {parseFloat(declaration.lignes[0].quantite_declaree)} {declaration.lignes[0].unite_mesure}</p>
                  <p className="text-sm font-medium text-slate-300"><strong>Poids Brut:</strong> {declaration.poids_brut_kg?.toFixed(2) || 'Non spécifié'} kg</p>
                  <p className="text-sm font-medium text-slate-300"><strong>Poids Net:</strong> {declaration.poids_net_kg?.toFixed(2) || 'Non spécifié'} kg</p>
                  <p className="text-sm font-medium text-slate-300"><strong>Volume:</strong> {declaration.volume_m3?.toFixed(2) || 'Non spécifié'} m³</p>
                  <p className="text-sm font-medium text-slate-300"><strong>Nombre de Colis:</strong> {declaration.nombre_colis || 'Non spécifié'}</p>
                </>
              )}
            </div>
            {declaration.description_marchandises && (
              <div className="mt-4">
                <p className="text-sm font-medium text-slate-300 mb-1">Description Détaillée</p>
                ...
                <table className="min-w-full divide-y divide-slate-800">
                  ...
                  <tbody className="bg-slate-900 divide-y divide-slate-800">
                    {declaration.receptions.map((rec: any) => (
                      <tr key={rec.id} className="hover:bg-slate-800">
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${
                            rec.statut === 'VALIDEE' ? 'bg-emerald-500/15 text-emerald-300' :
                            rec.statut === 'EN_COURS' ? 'bg-blue-500/15 text-blue-300' :
                            rec.statut === 'ANNULEE' ? 'bg-red-500/15 text-red-300' :
        ...
        <div className="mb-8">
          ...
          <div className="bg-slate-900 rounded-2xl shadow-sm border border-slate-700 p-6">
            {timingLoading ? (
              ...
              <>
                <div className="mb-4">
                  <p className="text-sm font-medium text-slate-300"><strong>Date Estimée de Réception Complète:</strong></p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm font-medium text-slate-300"><strong>Basé sur l'historique:</strong></p>
                    <p className="text-sm text-slate-500">
                      {receptionTiming.based_on_history ? 'Oui' : 'Non (utilisation de la valeur par défaut de 7 jours)'}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-slate-300"><strong>Durée moyenne historique:</strong></p>
                    <p className="text-sm text-slate-500">
                      {receptionTiming.average_duration_days?.toFixed(1)} jours
                    </p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-sm font-medium text-slate-300"><strong>Taille de l'échantillon:</strong></p>
                    <p className="text-sm text-slate-500">
                ...
                {(!receptionTiming.based_on_history || receptionTiming.sample_size === 0) && (
                  <div className="mt-4 p-3 bg-yellow-500/10 rounded-lg border border-yellow-500/40">
