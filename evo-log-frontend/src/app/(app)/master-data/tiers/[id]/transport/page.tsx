...
export default function ClientTransportHistoryPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // apiClient : base + token reel + normalization /api -> /api/v1.
        // Fiche tiers reelle (/api/v1/tiers/{id}) et missions vraiment
        // rattachees au client (/api/v1/transport/missions/client/{id}).
        const [clientRes, missionsRes] = await Promise.all([
          apiClient.get(`/api/tiers/${id}`).catch(() => null),
          apiClient.get(`/api/transport/missions/client/${id}`).catch(() => null),
        ])
        if (clientRes?.data) setClient(clientRes.data)
        if (Array.isArray(missionsRes?.data)) setMissions(missionsRes.data)
      } catch (err) {
        console.error(err)
      } finally {
        setLoading(false)
  ...
  // Mission backend : valeurs reellement stockees (cout_reel, sinon estime).
  const totalCA = missions.reduce((acc, m) => acc + (parseFloat(m.cout_reel ?? m.cout_estime ?? m.montant_fret) || 0), 0)
      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
        ...
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Missions effectuées pour le client <strong className="text-slate-200">{client.raison_sociale || client.name}</strong></p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-700 flex items-center gap-4">
            <div className="p-4 bg-blue-500/15 text-blue-600 rounded-xl"><Truck className="w-6 h-6" /></div>
            <div>
              <p className="text-sm font-bold text-slate-500">Total Missions</p>
              <h3 className="text-2xl font-black text-slate-200">{missions.length}</h3>
            </div>
          </div>
          <div className="bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-700 flex items-center gap-4">
            <div className="p-4 bg-emerald-500/15 text-emerald-600 rounded-xl"><CircleDollarSign className="w-6 h-6" /></div>
            <div>
              <p className="text-sm font-bold text-slate-500">Chiffre d'Affaires Transport</p>
              <h3 className="text-2xl font-black text-slate-200">{totalCA.toLocaleString()} FCFA</h3>
            </div>
          </div>
        </div>
              ...
              {missions.length === 0 ? (
                ...
                <tr key={mission.id} className="hover:bg-slate-800/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-black text-slate-200">OT-{mission.id.toString().padStart(4, '0')}</div>
                  ...
                  <td className="px-6 py-4 font-black text-slate-200">
                    {(() => { const c = parseFloat(mission.cout_reel ?? mission.cout_estime ?? mission.montant_fret); return Number.isFinite(c) && c > 0 ? `${c.toLocaleString()} FCFA` : '-' })()}
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${mission.statut === 'TERMINEE' || mission.statut === 'FACTUREE' ? 'bg-emerald-500/15 text-emerald-300' : 'bg-amber-500/15 text-amber-300'}`}>
