...
export default function EnterpriseChatPage() {
  ...
  // Refresh only when the shared authenticated WebSocket announces an event.
  useEffect(() => {
    const handleRealtimeEvent = (event: Event) => {
      ...
      if (activeTab === 'direct' && selectedColleague) {
        ...
        if (!data.room_id || data.room_id === selectedRoom.id || data.room_uuid === selectedRoom.room_uuid) {
          fetchRoomMessages(selectedRoom.room_uuid);
        }
      }
    };
    window.addEventListener('evo-log:event', handleRealtimeEvent);
    return () => window.removeEventListener('evo-log:event', handleRealtimeEvent);
