...
import { apiClient, transportAPI } from '@/lib/api-client'
...
export default function DriverProfilePage() {
  ...
  useEffect(() => {
    const fetchDriverData = async () => {
      try {
        // Via apiClient : base resolvee, token NextAuth reel, /api -> /api/v1.
        const [driverRes, missionsRes, docsRes] = await Promise.all([
          apiClient.get(`/api/transport/chauffeurs/${id}`).catch(() => null),
          apiClient.get(`/api/transport/missions/chauffeur/${id}`).catch(() => null),
          apiClient.get(`/api/transport/chauffeurs/${id}/documents`).catch(() => null),
        ])

        if (driverRes?.data) setDriver(driverRes.data)
        const mRows = missionsRes?.data
        if (Array.isArray(mRows)) setMissions(mRows)
        const dRows = docsRes?.data?.documents
        if (Array.isArray(dRows)) setDocuments(dRows)
      } catch (err) {
        console.error(err)
      } finally {
        setLoading(false)
  ...
  const activeMissions = missions.filter(m => ['EN_ROUTE', 'AFFECTEE', 'en_cours', 'planifiee'].includes(String(m.statut)))
  const completedMissions = missions.filter(m => ['TERMINEE', 'FACTUREE', 'terminee'].includes(String(m.statut)))
    ...
    <ModuleLayout module="transport">
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
        ...
        <div className="mb-8 flex items-center gap-4">
          <button onClick={() => router.back()} className="p-2 bg-slate-900 border border-slate-700 rounded-xl hover:bg-slate-800 text-slate-500 hover:text-slate-300 transition-colors">
          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            <div className="bg-slate-900 rounded-2xl shadow-sm border border-slate-700 p-6">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-16 h-16 bg-blue-500/15 text-blue-600 rounded-full flex items-center justify-center text-2xl font-bold">
                  {driver.nom.charAt(0)}{driver.prenom.charAt(0)}
                </div>
                <div>
                  <h2 className="text-xl font-bold text-slate-200">{driver.nom} {driver.prenom}</h2>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center py-3 border-b border-slate-700">
                  ...
                  <span className="text-slate-200 font-bold">{driver.telephone}</span>
                </div>
                <div className="flex justify-between items-center py-3 border-b border-slate-700">
                  <span className="text-slate-500 font-medium text-sm">Date d'embauche</span>
                  <span className="text-slate-200 font-bold">{new Date(driver.date_embauche).toLocaleDateString()}</span>
                </div>
                <div className="flex justify-between items-center py-3">
                  <span className="text-slate-500 font-medium text-sm">Statut</span>
                  {driver.statut === 'DISPONIBLE' ? (
                    <span className="px-3 py-1 bg-emerald-500/15 text-emerald-300 rounded-full text-xs font-bold flex items-center gap-1">
                      <CheckCircle2 className="w-3.5 h-3.5" /> Disponible
                    </span>
                  ) : (
                    <span className="px-3 py-1 bg-blue-500/15 text-blue-300 rounded-full text-xs font-bold flex items-center gap-1">
              ...
              <div className="p-5">
                {documents.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {documents.map(doc => {
                      const expired = Boolean(doc.date_expiration) && isExpired(doc.date_expiration);
                      return (
                        <div key={doc.id || doc.type} className={`p-4 rounded-xl border ${expired ? 'bg-red-500/10 border-red-500/40' : 'bg-slate-900 border-slate-700'}`}>
                          <div className="flex items-center gap-3 mb-2">
                            {expired ? <ShieldAlert className="w-5 h-5 text-red-600" /> : <CheckCircle2 className="w-5 h-5 text-emerald-600" />}
                            <h4 className="font-bold text-slate-200">{doc.type_document || doc.libelle || doc.type}</h4>
                          </div>
                          <p className="text-sm font-bold text-slate-500 mb-2">{doc.numero || doc.valeur || ''}</p>
              ...
              <div className="divide-y divide-slate-800 max-h-[400px] overflow-y-auto custom-scrollbar">
                {missions.length === 0 ? (
                  ...
                  missions.sort((a, b) => b.id - a.id).map(mission => (
                    <div key={mission.id} className="p-5 hover:bg-slate-800 transition-colors">
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex items-center gap-2">
                          <span className="font-black text-lg text-slate-200">OT-{mission.id.toString().padStart(4, '0')}</span>
                          <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${mission.statut === 'TERMINEE' || mission.statut === 'FACTUREE' ? 'bg-emerald-500/15 text-emerald-300' : 'bg-amber-500/15 text-amber-300'}`}>
                      ...
                      <div className="flex items-center gap-6 text-sm">
                        ...
                        <div className="flex-1 border-l-2 border-slate-700 pl-6 space-y-2">
                          {mission.prix_carburant_prevu && (
                            <div className="flex justify-between">
                              <span className="text-slate-500 text-xs">Frais Carburant Prévus</span>
                              <span className="font-bold text-slate-200 text-xs">{mission.prix_carburant_prevu} FCFA</span>
                            </div>
                          )}
                          <div className="flex justify-between">
                            <span className="text-slate-500 text-xs">Frais Route Prévus</span>
                            <span className="font-bold text-slate-200 text-xs">{mission.frais_route_prevus} FCFA</span>
                          </div>
                        </div>
                      </div>
                    </div>
