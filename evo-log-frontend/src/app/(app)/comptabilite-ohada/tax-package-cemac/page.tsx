...
export default function ComptabiliteOhadaTaxPackageCemac() {
  ...
  const tvaCollectee = tvaData?.tva_collectee ?? 7_200_000;
  const caTaxable = tvaData?.ca_taxable ?? 37_402_597;
  const tvaDeductibleImmo = tvaData?.tva_deductible_immo ?? 1_200_000;
  const tvaDeductibleCharges = tvaData?.tva_deductible_charges ?? 2_180_000;
  const creditAnterieur = tvaData?.credit_tva_anterieur ?? 0;
  const operationsExonerees = tvaData?.operations_exonerees ?? 64_797_403;
