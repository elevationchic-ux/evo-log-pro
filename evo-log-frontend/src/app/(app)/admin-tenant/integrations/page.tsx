...
export default function AdminTenantIntegrationsPage() {
  ...
  const handleTestConnection = (connector: IntegrationConnector) => {
    setTestingId(null);
    toast.error(`Test indisponible : la connexion réelle à ${connector.name} n’est pas configurée dans cet environnement.`);
  };

  const handleSaveConfig = (e: React.FormEvent) => {
    e.preventDefault();
    toast.error(`Configuration indisponible : aucun endpoint persistant n’est configuré pour ${selectedConnector?.name}.`);
