...
export default function B2BBrandingPage() {
  ...
  const { data: portal } = useQuery({
    queryKey: ['b2b-portal', companyId],
    queryFn: async () => {
      const res = await apiClient.get(`/api/v1/b2b/portal/${companyId}`)
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      const res = await apiClient.put(`/api/v1/b2b/portal/${companyId}`, data)
      return res.data
    },
    onSuccess: () => {
      console.log('Portail personnalisé avec succès')
      queryClient.invalidateQueries({ queryKey: ['b2b-portal', companyId] })
    },
    onError: () => {
