  ...
  const loadClaims = async () => {
    ...
    try {
      ...
      if (Array.isArray(raw) && raw.length > 0) {
        setClaims(raw.filter((i: any) => i.id != null).map((i: any) => ({
          id: i.id.toString(),
  ...
  const handleCreateClaim = async (e: React.FormEvent) => {
    ...
    try {
      await supportAPI.createIncident({
        ...
        type: form.typeLitige,
        priorite: form.priorite,
        description: form.description,
        montant: Number(form.montantReclame) || 0,
      });
      await loadClaims();
      setShowAddModal(false);
      setForm({ reference: '', clientNom: '', typeLitige: 'AVARIE_MARCHANDISE', priorite: 'HAUTE', description: '', montantReclame: '' });
      toast.success('La réclamation a été enregistrée.');
    } catch (err: any) {
      toast.error(err?.response?.data?.detail || 'La réclamation n’a pas pu être enregistrée.');
    }
  };
