...
export default function ClientB2bCrmPage() {
  ...
  const loadContacts = async () => {
    ...
    try {
      ...
      if (Array.isArray(raw) && raw.length > 0) {
        setContacts(raw.filter((c: any) => c.id != null).map((c: any) => ({
          id: c.id.toString(),
  ...
  const handleCreateContact = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ nomEntreprise: '', contactNom: '', poste: 'Directeur Logistique / Supply Chain', telephone: '', email: '', ville: '', pays: '' });
      toast.success(`Contact « ${form.contactNom} » ajouté avec succès.`);
    } catch (err: any) {
      toast.error(err?.response?.data?.detail || 'Le contact n’a pas pu être enregistré.');
    }
  };
