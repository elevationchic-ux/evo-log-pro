...
export default function B2BLandingPage() {
  ...
  const handleClientLogin = (e: React.FormEvent) => {
    ...
    setIsLoggingIn(true);
    authAPI.login({ username: clientEmail, password })
      .then(() => {
        toast.success('Connexion réussie au portail chargeur !');
        router.push('/b2b/dashboard');
      })
      .catch(() => toast.error('Identifiants invalides ou portail non configuré.'))
              ...
              <form onSubmit={handleQuickTrack} className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder="Saisissez une référence de suivi"
          ...
          <div className="lg:col-span-5">
            <div className="bg-slate-900/95 border border-slate-800 rounded-3xl p-8 shadow-2xl backdrop-blur-2xl relative">
              ...
              <form onSubmit={handleClientLogin} className="space-y-4">
                ...
                {/* Tenant is determined by the authenticated account. */}
                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5">
                    Société du compte
                  </label>
                  <div className="relative h-11 pl-10 flex items-center bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-400">
                    <Building2 className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
                    Déterminée après authentification
                  </div>
                </div>
