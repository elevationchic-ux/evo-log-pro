...
import { ChevronRight, ChevronLeft, X, Lock, AlertTriangle } from 'lucide-react';
...
export default function ModuleSidebar({
    ...
    <div className="flex flex-col h-full bg-slate-900 border-r border-slate-800 text-slate-300 w-full select-none">
      ...
      <div className="flex items-center justify-between h-16 px-4 border-b border-slate-800 shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-amber-500 text-slate-950 font-black flex items-center justify-center shadow-lg shadow-amber-500/30 text-sm">
            E
          </div>
          {!isCollapsed && (
            <div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1">
        {filteredNav.map((item) => {
          const rootPathPrefix = `/${item.path.split('/')[1]}`;
          ...
          if (!isAllowed) {
            return (
              <div
                ...
                className={`flex items-center gap-3 py-2.5 rounded-xl text-sm font-semibold opacity-40 bg-slate-950/40 border border-slate-800 text-slate-500 cursor-not-allowed transition-all hover:opacity-65 ${
                  isCollapsed ? 'px-2 justify-center' : 'px-3'
                }`}
              >
                <Icon className="w-5 h-5 shrink-0 text-slate-600" />
                {!isCollapsed && <span className="truncate flex-1 text-slate-500 line-through decoration-slate-600">{item.title}</span>}
                {!isCollapsed && <Lock className="w-3.5 h-3.5 text-amber-500/80 shrink-0" />}
              </div>
            );
          }
              ...
              <div className="flex items-center">
                <Link
                  ...
                  className={`flex-1 flex items-center gap-3 py-2.5 rounded-xl text-sm font-semibold transition-all group ${
                    ...
                    isActive
                      ? 'text-white border border-slate-600/50 shadow-sm'
                      : 'text-slate-400 hover:text-slate-100 hover:bg-slate-800/60'
                  }`}
                  style={isActive ? { backgroundColor: `${item.color}20`, borderColor: `${item.color}40` } : {}}
                >
                  <Icon
                    className="w-5 h-5 shrink-0 transition-colors"
                    style={isActive ? { color: item.color } : {}}
                  />
                  {!isCollapsed && <span className="truncate flex-1" style={isActive ? { color: item.color } : {}}>{item.title}</span>}
                </Link>
                ...
                {!isCollapsed && subItems.length > 0 && (
                  ...
                  >
                    <ChevronRight
                      ...
                      style={isAccordionOpen ? { transform: 'rotate(90deg)', color: item.color } : {}}
                    />
                  </button>
                )}
              </div>

              {/* Accordion Sub-modules */}
                  ...
                  {subItems.map((sub) => {
                    ...
                    return (
                      <Link
                        ...
                        style={isSubActive ? { backgroundColor: `${item.color}25`, color: item.color, borderColor: `${item.color}40` } : {}}
                      >
          ...
          <div className="bg-slate-900 border border-amber-500/40 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4 animate-in zoom-in-95 duration-200 text-slate-100">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-slate-300 leading-relaxed font-medium bg-slate-950 p-3 rounded-2xl border border-slate-800">
                Accès restreint : Votre profil{' '}
                <b className="text-amber-400">[{userRoles.join(', ') || 'Utilisateur'}]</b>{' '}
                n&apos;est pas autorisé à accéder au module{' '}
                <b className="text-white">[{deniedModalItem.label}]</b>. Veuillez contacter l&apos;Admin CADC.
              </p>
  ...
  if (isMobile) {
    ...
    return <div className="fixed inset-y-0 left-0 z-[60] w-72 shadow-2xl">{sidebarContent}</div>;
  }
