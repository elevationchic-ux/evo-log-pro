...
AcquisitionService = AcquisitionReportingService
AppelOffreService = AppelOffresService


def _legacy_appel_offre_creer(db: Session, numero_appel: str, titre: str, type_appel: str,
                             date_limite: date, budget_estime: float) -> AppelOffres:
    if isinstance(type_appel, TypeAppelOffres):
        normalized_type = type_appel
    else:
        raw_type = str(type_appel).strip()
        try:
            normalized_type = TypeAppelOffres[raw_type.upper()]
        except KeyError:
            try:
                normalized_type = TypeAppelOffres(raw_type.lower())
            except ValueError as exc:
                raise ValueError(f"Type d'appel d'offres invalide: {type_appel}") from exc
    appel = AppelOffres(
        numero_appel=numero_appel,
        titre=titre,
        type_appel=normalized_type,
        statut=StatutAppelOffres.PUBLIE,
...
AppelOffresService.creer_appel_offre = staticmethod(_legacy_appel_offre_creer)


def _legacy_appel_lancer_selection(db: Session, appel_id: int) -> AppelOffres:
    appel = db.query(AppelOffres).filter(AppelOffres.id == appel_id).first()
    if not appel:
        raise ValueError("Appel d'offres non trouvé")
    appel.statut = StatutAppelOffres.EN_SELECTION
    db.commit()
    db.refresh(appel)
    return appel


AppelOffresService.lancer_selection = staticmethod(_legacy_appel_lancer_selection)


def _legacy_contrat_creer(db: Session, numero_contrat: str, fournisseur_id: int, type_contrat: str,
                         date_debut: date, date_fin: date, montant_max: float) -> ContratCadre:
    duree_mois = ((date_fin.year - date_debut.year) * 12) + (date_fin.month - date_debut.month)
    contrat = ContratCadre(
        numero_contrat=numero_contrat,
        fournisseur_id=fournisseur_id,
...
ContratCadreService.creer_contrat_cadre = staticmethod(_legacy_contrat_creer)


def _legacy_bc_creer(db: Session, numero_bc: str, fournisseur_id: int, contrat_cadre_id: int,
                    date_commande: date, montant_ht: float) -> BonCommande:
    bc = BonCommande(
        numero_bc=numero_bc,
        fournisseur_id=fournisseur_id,
        contrat_cadre_id=contrat_cadre_id,
...
BonCommandeService.creer_bon_commande = staticmethod(_legacy_bc_creer)


def _legacy_reception_creer(db: Session, numero_reception: str, bon_commande_id: int,
                          date_reception: date, quantite_recue: float, quantite_commandee: float) -> Reception:
    reception = Reception(
        numero_reception=numero_reception,
        bc_id=bon_commande_id,
        fournisseur_id=1,
        date_reception=date_reception,
        type_reception="complete",
        lieu_reception="Entrepot",
        responsable="system",
        condition_marchandise="conforme",
        statut="conforme",
        nombre_colis=int(quantite_recue),
    )
    db.add(reception)
    db.commit()
    db.refresh(reception)
...
ReceptionService.creer_reception = staticmethod(_legacy_reception_creer)
