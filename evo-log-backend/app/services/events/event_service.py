...
class ReactiveEventBus:
    ...
    async def _ensure_started(self):
        """Ensure Redis is connected; silently skip if running in degraded mode."""
    ...
    async def emit_tenant_event(self, company_id, event_type, data, target_departments=None, target_users=None):
        ...
        if self.redis is not None:
            try:
                ...
                logger.warning("Redis publish failed (degraded mode): %s", exc)
