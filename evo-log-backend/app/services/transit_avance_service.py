...
TransitAvanceService = DossierTransitAvanceService
DossierTransitService = DossierTransitAvanceService
ValorisationService = ValorisationDouaniereService
BADService = BonADService


def _legacy_dossier_creer(db: Session, numero_dossier: str, bureau_entree_id: int,
                         bureau_sortie_id: int, type_transit: str,
                         reference_externe: str = None) -> DossierTransitAvance:
    dossier = DossierTransitAvance(
        numero_dossier=numero_dossier,
        client_id=1,
        transitaire_id=1,
        type_transit=TypeTransit[type_transit.upper()] if isinstance(type_transit, str) and type_transit.upper() in TypeTransit.__members__ else TypeTransit.ORDINAIRE,
        regime_douanier=RegimeDouanier.ENTREPOT_DOUANIER,
        bureau_entree_id=bureau_entree_id,
        bureau_sortie_id=bureau_sortie_id,
        marchandise=reference_externe or "Marchandise",
        valeur_marchandise=0,
        pays_origine_code="CM",
        pays_destination_code="CM",
        statut="en_cours",
    )
    dossier.reference_externe = reference_externe or "REF-DEFAULT"
    db.add(dossier)
    db.commit()
    db.refresh(dossier)
    return dossier
...
DossierTransitAvanceService.creer_dossier_transit = staticmethod(_legacy_dossier_creer)


def _legacy_visite_planifier(db: Session, numero_visite: str, dossier_transit_id: int,
                            date_visite: date, inspecteur_id: int) -> VisitePhysique:
    visite = VisitePhysique(
        dossier_transit_id=dossier_transit_id,
        date_visite=datetime.combine(date_visite, datetime.min.time()),
        inspecteur_id=inspecteur_id,
        type_visite="controle",
        rapport="Visite planifiée",
        conformite=False,
        resultat="planifie",
    )
    visite.numero_visite = numero_visite
...
VisitePhysiqueService.planifier_visite = staticmethod(_legacy_visite_planifier)


def _legacy_valorisation_creer(db: Session, numero_valorisation: str, dossier_transit_id: int,
                              valeur_cif: float, valeur_douane: float, taux_change: float) -> ValorisationDouaniere:
    valorisation = ValorisationDouaniere(
        dossier_transit_id=dossier_transit_id,
        methode_valorisation="transaction",
        valeur_caf=valeur_cif,
        fret=0,
        assurance=0,
        autres_frais=0,
        valeur_fob=valeur_cif,
        taux_change=taux_change,
        devise="XAF",
        date_valorisation=date.today(),
        valide_par=1,
    )
    valorisation.numero_valorisation = numero_valorisation
    valorisation.valeur_cif = valeur_cif
    valorisation.valeur_douane = valeur_douane
    valorisation.statut = "valide"
    db.add(valorisation)
    db.commit()
    db.refresh(valorisation)
    return valorisation
...
ValorisationDouaniereService.creer_valorisation = staticmethod(_legacy_valorisation_creer)


def _legacy_bad_emettre(db: Session, numero_bad: str, dossier_transit_id: int,
                       date_emission: date, date_validite: date) -> BonAD:
    bad = BonAD(
        numero_bad=numero_bad,
        dossier_transit_id=dossier_transit_id,
        declaration_id=1,
        signataire="system",
        qualite="conforme",
        date_signature=date_emission,
        statut="emis",
    )
    bad.date_emission = date_emission
    bad.date_validite = date_validite
    db.add(bad)
    db.commit()
    db.refresh(bad)
    return bad
...
BonADService.emettre_bad = staticmethod(_legacy_bad_emettre)
