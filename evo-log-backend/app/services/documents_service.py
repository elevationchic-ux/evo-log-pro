...
class DocumentService:
    ...
    def creer_document(
        ...
        type_document: TypeDocument | str = TypeDocument.AUTRE,
        titre: str = "",
        proprietaire_id: Optional[int] = None,
        dossier_id: Optional[int] = None,
        fichier: Optional[bytes] = None,
        ...
        """Create document"""
        numero_document = numero_document or reference
        if not numero_document:
            raise ValueError("Le numéro ou la référence du document est obligatoire")
        if isinstance(type_document, str):
            try:
                type_document = TypeDocument[type_document.upper()]
            except KeyError:
                try:
                    type_document = TypeDocument(type_document.lower())
                except ValueError as exc:
                    raise ValueError(f"Type de document invalide: {type_document}") from exc
        fichier = fichier or b""
        document = Document(
            ...
            type_mime=type_mime or mime_type,
            taille_octets=taille_octets if taille_octets is not None else len(fichier),
            emplacement_stockage=chemin,
