...
SurestariesService = SurestarieService


def _legacy_stowage_plan_creer(db: Session, numero_plan: str, navire: str, voyage: str,
                              date_arrivee: date, position_actuelle: str) -> StowagePlan:
    vessel = db.query(Navire).filter(Navire.nom == navire).first()
    if vessel is None:
        vessel = Navire(nom=navire, is_active=True)
        db.add(vessel)
        db.flush()
    plan = StowagePlan(
        navire_id=vessel.id,
    ...
    plan.numero_plan = numero_plan
    plan.statut = "planifie"
    plan.position_actuelle = position_actuelle
    db.add(plan)
    db.commit()
    db.refresh(plan)
    return plan
...
StowagePlanService.creer_stowage_plan = staticmethod(_legacy_stowage_plan_creer)


def _legacy_grue_creer(db: Session, code_grue: str, nom: str, type_grue: str,
                      capacite_max: float, fabricant: str) -> Grue:
    grue = Grue(
        code=code_grue,
        type_grue=type_grue,
        capacite_tonnes=capacite_max,
        portee_metres=0,
        hauteur_metres=0,
        poste_quai="",
        statut="actif",
    )
    grue.code_grue = code_grue
    grue.nom = nom
    grue.fabricant = fabricant
    db.add(grue)
    db.commit()
    db.refresh(grue)
    return grue
...
GrueService.creer_grue = staticmethod(_legacy_grue_creer)


def _legacy_connaissement_creer(db: Session, numero_bl: str, navire: str, voyage: str,
                               port_chargement: str, port_dechargement: str, nombre_conteneurs: int) -> Connaissement:
    bl = Connaissement(
        numero_bl=numero_bl,
        conteneur_id=1,
        type_bl="direct",
        chargeur=navire,
        destinataire=voyage,
        port_embarquement=port_chargement,
        port_dechargement=port_dechargement,
        date_emission=date.today(),
        montant_freight=0,
        devise="XAF",
    ...
    bl.navire = navire
    bl.voyage = voyage
    bl.port_chargement = port_chargement
    bl.port_dechargement = port_dechargement
    bl.nombre_conteneurs = nombre_conteneurs
    db.add(bl)
    db.commit()
    db.refresh(bl)
    return bl
...
ConnaissementService.creer_connaissement = staticmethod(_legacy_connaissement_creer)


def _legacy_surestaries_calculer(db: Session, numero_surestaries: str, connaissement_id: int,
                                date_arrivee: date, date_depart: date, taux_journalier: float) -> Surestarie:
    nombre_jours = (date_depart - date_arrivee).days
    montant_total = nombre_jours * taux_journalier
    surestarie = Surestarie(
        connaissement_id=connaissement_id,
    ...
    surestarie.numero_surestaries = numero_surestaries
    db.add(surestarie)
    db.commit()
    db.refresh(surestarie)
    return surestarie
...
SurestarieService.calculer_surestaries = staticmethod(_legacy_surestaries_calculer)
