...
class B2BService:
    ...
    def get_client_invoices(db: Session, company_id: int):
        ...
        return db.query(Facture).filter(Facture.company_id == company_id).all()
    
    @staticmethod
    def get_client_shipments(db: Session, company_id: int):
        """Get shipments for a specific company (B2B isolation)"""
        if not hasattr(Mission, "company_id"):
            raise ValueError("Le modèle des missions ne supporte pas encore l'isolation par société")
        return db.query(Mission).filter(Mission.company_id == company_id).all()
...
class B2BReportingService:
    ...
    def rapport_personnalise(db: Session, company_id: int, type_rapport: str):
        ...
        if type_rapport not in {"activite", "financier"}:
            raise ValueError(f"Type de rapport non supporté: {type_rapport}")

        invoices = db.query(Facture).filter(Facture.company_id == company_id).all()
        kpis: Dict[str, Any] = {
            "factures": len(invoices),
            "chiffre_affaires": sum((invoice.montant_ttc or 0) for invoice in invoices),
            "factures_impayees": sum(
                1 for invoice in invoices
                if getattr(invoice.statut, "value", invoice.statut) not in {"payee", "PAYEE"}
            ),
        }
        if type_rapport == "activite":
            kpis["livraisons"] = len(B2BService.get_client_shipments(db, company_id))
