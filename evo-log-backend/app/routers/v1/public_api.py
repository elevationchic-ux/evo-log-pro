...
from app.models.conteneur_cycle import ConteneurCycle as Conteneur
...
async def public_track_shipment(
    ...
    conteneur = db.query(Conteneur).filter(
        Conteneur.numero.ilike(f"%{reference}%")
    ).first()
    if conteneur:
        return {
            "type": "conteneur",
            "reference": conteneur.numero,
            "statut": conteneur.etat.value if hasattr(conteneur.etat, "value") else str(conteneur.etat),
            "type_conteneur": conteneur.type_conteneur,
            "taille": conteneur.taille_pieds,
        }
