...
class TenantMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if authenticated_user and not authenticated_user.is_superuser:
            authenticated_company_id = authenticated_user.company_id
            if tenant_id is not None and int(tenant_id) != authenticated_company_id:
                return JSONResponse(
                    {"detail": "Tenant header does not match authenticated user"},
                    status_code=status.HTTP_403_FORBIDDEN,
                )
            tenant_id = str(authenticated_company_id) if authenticated_company_id is not None else None

        # Inject tenant into request state
        if tenant_id:
            request.state.tenant_id = tenant_id
            request.state.company_id = int(tenant_id)
            request.state.department_id = (
                authenticated_user.department_id if authenticated_user else None
            )
        else:
