...
5. **Tests fiscaux tautologiques** : les tests recodent la formule du service au lieu de vérifier des montants attendus du barème officiel → réécrire sur valeurs de référence publiées.
...
7. ~~**Dossier unique de marchandise** (BL → déclaration → acconage → magasin → livraison → facture)~~ → **VUE CONSOLIDÉE HONNÊTE LIVRÉE (batch 6, voir §8)** : `GET /api/v1/acconage/dossier-marchandise` retrace tout ce qui est **réellement relié** (clé étrangère ou numéro physique de conteneur) et signale explicitement `non_liciable_en_base` les étapes que le schéma ne relie pas encore  sans jamais inventer de liaison. ⏳ **Écart d'architecture restant** : la déclaration douanière, le magasin, la mission et la facture ne portent aucune colonne `numero_conteneur`/`bl` ; un `dossier_id` (ou référence conteneur) est à ajouter pour fermer la chaîne de bout en bout.
